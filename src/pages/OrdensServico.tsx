...
const OrdensServico = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const numericFields = ['parcelas', 'juros'];
    setFormData(prev => ({
      ...prev,
  ...
  const handleMultiSelectChange = (e: SelectChangeEvent<string[]>, field: 'servicosIds' | 'pecasIds') => {
  ...
  // Calcular valor total baseado nos serviços, peças, juros selecionados
  useEffect(() => {
    let total = 0;
    ...
    const valorComJuros = total * (1 + formData.juros / 100);
    setFormData(prev => ({
      ...prev,
      valorBase: total,
      valorTotal: valorComJuros
    }));
  }, [formData.servicosIds, formData.pecasIds, formData.juros, servicos, pecas]);
  ...
  const restaurarEstoque = async (pecasIds: string[]) => {
    for (const pecaId of pecasIds) {
      const peca = pecas.find(p => p.id === pecaId);
      if (peca) {
        await axios.put(`http://localhost:3001/pecas/${pecaId}`, {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const newParcelas = formData.parcelas || 1;
      const ordemAntiga = editingId ? ordens.find(o => o.id === editingId) : undefined;
      const existingStatus = ordemAntiga?.parcelasStatus ?? [];
  ...
  const handleDelete = async () => {
    if (ordemParaDeletar) {
      try {
        const ordemADeletar = ordens.find(o => o.id === ordemParaDeletar);
        if (ordemADeletar) {
  ...
  const isEmAtraso = (ordem: OrdemServico): boolean => {
    const status = ordem.parcelasStatus;
    if (!status || status.length === 0) return false;
    return status.some(s => !s);
  };

  const handleToggleParcela = async (ordem: OrdemServico, index: number) => {
    const parcCount = ordem.parcelas ?? 1;
    const current = ordem.parcelasStatus ?? Array(parcCount).fill(false);
    const updated = [...current];
    updated[index] = !updated[index];
    try {
      await axios.patch(`http://localhost:3001/ordens_servico/${ordem.id}`, { parcelasStatus: updated });
      fetchData();
    } catch {
  ...
  const handlePrintOrdem = (ordem: OrdemServico) => {
    ...
    const logoUrl = `${window.location.origin}${HirataLogo}`;

    const valorBase = servicosSelecionados.reduce((acc, s) => acc + s.valor, 0) +
      pecasSelecionadas.reduce((acc, p) => acc + p.preco, 0);
    const juros = ordem.juros ?? 0;
    const parcelas = ordem.parcelas ?? 1;
    const valorParcela = parcelas > 0 ? ordem.valorTotal / parcelas : ordem.valorTotal;
    ...
    const printContent = `
      ...
      <body>
        ...
        ${juros > 0 ? `<div class="total" style="font-size:14px">Juros: ${juros}%</div>` : ''}
        <div class="total">
          Valor Total: ${formatCurrency(ordem.valorTotal)}
        </div>
        ${parcelas > 1 ? `<div class="total" style="font-size:14px">${parcelas}x de ${formatCurrency(valorParcela)}</div>` : ''}
            ...
            <TableBody>
              {ordensFiltered.length > 0 ? (
                ordensFiltered.map((ordem) => (
                  <TableRow
                    key={ordem.id}
                    ref={highlightId === String(ordem.id) ? highlightRef : null}
                    onClick={() => handleOpenForm(ordem)}
                    sx={{
                      cursor: 'pointer',
                      backgroundColor: highlightId === String(ordem.id)
                        ? '#fff9c4'
                        : isEmAtraso(ordem) ? '#ffebee' : undefined,
                      outline: highlightId === String(ordem.id) ? '2px solid #f9a825' : undefined,
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip
                        label={ordem.status}
                        color={getStatusChipColor(ordem.status)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Typography variant="body2">{formatCurrency(ordem.valorTotal)}</Typography>
                      {(ordem.parcelas ?? 1) >= 1 && (
                        <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', mt: 0.5 }}>
                          {Array.from({ length: ordem.parcelas ?? 1 }).map((_, i) => {
                            const pago = (ordem.parcelasStatus ?? [])[i] === true;
                            return (
                              <Chip
                                key={i}
                                label={(ordem.parcelas ?? 1) === 1
                                  ? (pago ? '✓ Pago' : '✗ Pendente')
                                  : `${i + 1}ª ${pago ? '✓' : '✗'}`}
                                size="small"
                                color={pago ? 'success' : 'error'}
                                variant="outlined"
                                onClick={(e) => { e.stopPropagation(); handleToggleParcela(ordem, i); }}
                    ...
                    <TableCell align="center" onClick={(e) => e.stopPropagation()}>
                      <IconButton
                        color="primary"
                        onClick={(e) => { e.stopPropagation(); handleOpenForm(ordem); }}
                        size="small"
                      >
                        <EditIcon />
                      </IconButton>
                      <IconButton
                        color="primary"
                        onClick={(e) => { e.stopPropagation(); handlePrintOrdem(ordem); }}
                        size="small"
                      >
                        <PrintIcon />
                      </IconButton>
                      <IconButton
                        color="error"
                        onClick={(e) => { e.stopPropagation(); handleOpenDelete(ordem.id); }}
                        size="small"
                      >
                        <DeleteIcon />
                      </IconButton>
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12}>
              <Box sx={{ p: 2, bgcolor: 'action.hover', borderRadius: 1, mt: 1 }}>
                ...
                <Typography variant="body1">Valor Base: <strong>{formatCurrency(formData.valorBase)}</strong></Typography>
                <Typography variant="body1">Valor Total (com juros): <strong>{formatCurrency(formData.valorTotal)}</strong></Typography>
                <Typography variant="body1">
                  Valor por Parcela: <strong>{formatCurrency(formData.parcelas > 0 ? formData.valorTotal / formData.parcelas : formData.valorTotal)}</strong>

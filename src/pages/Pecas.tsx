...
const Pecas = () => {
  ...
  useEffect(() => {
    ...
    if (filtro) {
      resultado = resultado.filter(peca =>
        peca.nome.toLowerCase().includes(filtro) ||
        peca.codigo.toLowerCase().includes(filtro) ||
        peca.marca.toLowerCase().includes(filtro) ||
        peca.preco.toString().includes(filtro) ||
        peca.quantidade.toString().includes(filtro)
      );
    }
    ...
    if (ordenacao.campo !== '') {
      ...
      resultado = [...resultado].sort((a, b) => {
        if (campo === 'preco' || campo === 'quantidade') {
          return ordenacao.direcao === 'asc' ? a[campo] - b[campo] : b[campo] - a[campo];
        }
  ...
  }, [pecas, filtro, ordenacao.campo, ordenacao.direcao]);
              ...
              {pecasFiltradas.length > 0 ? (
                pecasFiltradas.map((peca) => (
                  ...
                  >
                    ...
                    <TableCell>{formatCurrency(peca.preco)}</TableCell>
                    <TableCell>{peca.quantidade}</TableCell>
                    <TableCell align="center">
                      <IconButton
                        color="primary"
                        onClick={(e) => { e.stopPropagation(); handleOpenForm(peca); }}
                        size="small"
                      >
                        <EditIcon />
                      </IconButton>
                      <IconButton
                        color="error"
                        onClick={(e) => { e.stopPropagation(); handleOpenDelete(peca.id); }}
                        size="small"
                      >
                        <DeleteIcon />
                      </IconButton>
